void TwoBodyDecay() // Macro only
{
    ...
    * calculations and graphs hereonin are in GeV. */
    ...
    if (!gROOT -> GetClass("TGenPhaseSpace"))
     ...
     TH1F *hist = new TH1F("#psi(3770) Meson Decay", "Psi(3770) Decay", 50, 3, 4);
     ...
     for (Int_t n = 0; n < 100000; n++) // Loop 100,000 times
     {
         ...
         hist -> Fill(D_0D_0bar.M(), weight);
         // Fill histogram with invariant mass
     }
 
     TCanvas* canv = new TCanvas("canv", "Canvas", 500, 500);
 
     hist -> SetTitle("#psi(3770) Meson Decay;m(D^{0}#bar{D^{0}}) [GeV/c^{2}];");
     hist -> GetXaxis() -> CenterTitle();
     hist -> GetXaxis() -> SetTitleOffset(1.25);
     hist -> Draw();
 
     canv -> Print("TwoBodyDecay.eps");
}
