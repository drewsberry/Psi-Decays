...
void LHCbStyle(Bool_t colzPlot=kFALSE, Int_t NCont=25)
{
  ...
  cout << "executing LHCbStyle.h:" << endl;
  cout << "                      " << endl;
  cout << "                      " << endl;
  cout << "                         $      $   $   $$$   $    " << endl;
  cout << "                         $      $   $  $      $    " << endl;
  ...
  if (!colzPlot) lhcbStyle->SetPalette(1);
  ...
  if (colzPlot) {
    cout << "Setting style for color plots (nContours=" << NCont << ")" << endl;
    const Int_t NRGBs = 8;
    // const Int_t NCont = 999;
    
    // deep purple, dark blue, light blue, cyan, green, yellow, orange, red 
    //                        p    db    lb     c     g    y      o     r
//    Double_t stops[NRGBs] = { 0.0, 0.30, 0.5, 0.55, 0.70, 0.90, 0.95, 1.0 };
//    Double_t blue[NRGBs]  = { 0.75, 1.0,  0.9,  0.65,  0.0,  0.0,  0.0, 0.0 };
//    Double_t green[NRGBs] = { 0.20, 0.0,  0.8,  0.7,  0.55,  1.0,  0.4, 0.0 };
//    Double_t red[NRGBs]   = { 0.35, 0.0, 0.1,  0.1,   0.15,   1.0,  1.0, 1.0 };
    //                        p    db    lb     c     g      o     r
    Double_t stops[NRGBs] = { 0.0, 0.30, 0.5, 0.55, 0.70,  0.95, 1.0 };
    Double_t blue[NRGBs]  = { 0.75, 1.0,  0.9,  0.65,  0.0,  0.0, 0.0 };
    Double_t green[NRGBs] = { 0.20, 0.0,  0.8,  0.7,  0.55,  0.4, 0.0 };
    Double_t red[NRGBs]   = { 0.35, 0.0, 0.1,  0.1,   0.15,  1.0, 1.0 };
   
    //   Int_t* MyPalette = new Int_t[NCont];
    Int_t FI = TColor::CreateGradientColorTable(NRGBs, stops, red, green, blue,
                                                NCont);
    // for (int i=0;i<NCont;i++) MyPalette[i] = FI+i;
    //   lhcbStyle->SetPalette(NCont, MyPalette);
    //   delete MyPalette;
    //   MyPalette=0;
...
#endif // CHARMPRODSCRIPTS_LHCBSTYLE_H 
